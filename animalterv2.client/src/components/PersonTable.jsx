...
const PersonTable = ({data,editData}) => {
  return (
    <div>
        ...
        {data && (
            data?.map((element,i)=>(
              <div key={i} className='flex justify-between items-center border-b border-black py-2'>
                ...
                <p className='lg:w-1/10 xs:text-sm md:text-md lg:text-base xs:hidden lg:block text-center'>{element.roleId}</p>
                ...
                <div className='flex gap-3 '>
                  <i className='fa-solid fa-pen ' onClick={()=>{ editData(element.userId);}}></i>
                  <i className='fa-solid fa-trash xs:hidden md:flex' onClick={()=>{ editData(element.userId); }}></i>
                </div>
